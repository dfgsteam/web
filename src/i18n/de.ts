export const de = {
  lang: 'de',
  htmlLang: 'de-DE',
  meta: {
    title: 'Julius Hunold – Softwareentwickler & Prozessautomatisierung',
    description:
      'Maßgeschneiderte Web-Apps, Prozessautomatisierung und Digitalisierung mit PHP & Python – vom Konzept bis zum produktiven System. Softwareentwickler und technischer Projektmanager aus Sarstedt, Deutschland.',
    keywords: 'Julius Hunold, Softwareentwickler, Prozessautomatisierung, Digitalisierung, PHP, Python, Web-Apps, Portfolio, Projektmanagement',
  },
  nav: {
    about: 'Über mich',
    projects: 'Projekte',
    skills: 'Skills',
    services: 'Services',
    infra: 'Infra',
    cv: 'Lebenslauf',
    contact: 'Kontakt',
    blog: 'Blog',
    menu: 'Menü',
    close: 'Schließen',
  },
  hero: {
    links: [
      { label: 'Webdesign', url: 'https://hnld.de' },
      { label: 'Custom Apps', url: 'https://hunold24.de' },
    ],
    titleA: 'Ich automatisiere,',
    titleB: 'digitalisiere & entwickle.',
    titleHighlight: 'automatisiere,',
    subtitle:
      'Softwareentwickler mit Fokus auf maßgeschneiderte Web-Apps, Prozessautomatisierung und Digitalisierung – mit PHP & Python, vom Konzept bis zum Live-System. Seit über 8 Jahren an der Schnittstelle von Entwicklung, Automatisierung und technischem Projektmanagement.',
    primaryCta: 'Projekte ansehen',
    secondaryCta: 'Kontakt aufnehmen',
    scrollHint: 'Scrollen',
    stats: [
      { value: 50, label: 'Projekte mitgewirkt' },
      { value: 8, label: 'Jahre Erfahrung' },
      { value: 13, label: 'Technologien im Stack' },
    ],
  },
  portfolio: {
    label: 'Portfolio',
    heading: 'Zahlen & Fakten.',
    subheading:
      'Aus Projektarbeit, Freelance und Studium – ein paar Kennzahlen in der Übersicht.',
    imageAlt: 'Porträt von Julius Hunold',
    imageChip: 'Julius Hunold · Web-Developer',
    kpiProjects: 'Projekte mitgewirkt',
    kpiExperience: 'Jahre Erfahrung',
    kpiTechnologies: 'Technologien im Stack',
  },
  about: {
    label: 'Über mich',
    heading: 'Code ist die Brücke zwischen Idee und Realität.',
    paragraphs: [
      'Ich bin Softwareentwickler mit Schwerpunkt auf Web-Applikationen, Prozessautomatisierung und Digitalisierung – in PHP & Python, von der Anforderungsanalyse über die Implementierung bis zum produktiven Betrieb. Parallel verantworte ich als Projektleiter den technischen Glasfaser-NE4-Ausbau.',
      'Mit über 8 Jahren Praxiserfahrung habe ich Workflows automatisiert, manuelle Prozesse durch smarte Software ersetzt und Unternehmen bei der digitalen Transformation begleitet – pragmatisch, lösungsorientiert, konsequent.',
      'Ehrenamtlich engagiere ich mich als Gruppenleiter und im Media-Team der Schönstatt Mannesjugend, wo ich seit Jahren auch die Finanzverwaltung verantworte.',
    ],
    facts: [
      { icon: 'map-pin', label: 'Standort', value: 'Sarstedt, Deutschland' },
      { icon: 'sparkles', label: 'Alter', value: '23' },
      { icon: 'mail', label: 'E-Mail', value: 'info@julius-hunold.de' },
      { icon: 'phone', label: 'Telefon', value: 'auf Anfrage' },
    ],
    cta: 'Mehr erfahren',
  },
  workstyle: {
    label: 'AECdisc® Potenzialanalyse & KI-Workflows',
    heading: 'Stärken auf einen Blick.',
    subheading: 'Verhaltens- und Potenzialanalyse kombiniert mit KI-gestützten Entwicklungsprozessen.',
    strengths: [
      { icon: 'zap', title: 'Motivator', text: 'Energie und Enthusiasmus für Team und Ziel.' },
      { icon: 'layout-list', title: 'Organisator', text: 'Strukturierte Abläufe, klare Prioritäten.' },
      { icon: 'users', title: 'Führungsstärke', text: 'Verantwortung übernehmen und mitnehmen.' },
      { icon: 'target', title: 'Ergebnisorientiert', text: 'Fokus auf konkrete, messbare Ergebnisse.' },
      { icon: 'rocket', title: 'Eigeninitiative', text: 'Probleme erkennen und selbst anpacken.' },
      { icon: 'lightbulb', title: 'Lösungsorientierung', text: 'Pragmatische Lösungen statt Grübeln.' },
      { icon: 'book-open', title: 'Autodidaktik', text: 'Schnell in neue Themen einarbeiten.' },
      { icon: 'kanban', title: 'Projektkoordination', text: 'Parallele Projekte im Blick behalten.' },
      { icon: 'check-circle', title: 'Verlässlichkeit', text: 'Zusagen halten – auch unter Druck.' },
      { icon: 'sparkles', title: 'Analyse & Fokus', text: 'Daten und Fakten als Entscheidungsbasis.' },
    ],
  },
  projects: {
    label: 'Projekte',
    heading: 'Ausgewählte Arbeit.',
    subheading: 'Ein Einblick in Projekte aus Selbstständigkeit und Beruf.',
    all: 'Alle Projekte',
    viewDetails: 'Details ansehen',
    back: 'Zurück zu den Projekten',
    client: 'Auftraggeber',
    year: 'Jahr',
    tech: 'Technologien',
    overview: 'Überblick',
    category: {
      client: 'Kunden & Selbstständigkeit',
      work: 'Berufliche Arbeit',
    },
    clientLabel: {
      client: 'Auftraggeber',
      work: 'Arbeitgeber / Unternehmen',
    },
    status: {
      live: 'Live',
      'client-project': 'Kundenprojekt',
      'under-construction': 'In Entwicklung',
    },
    related: 'Weitere Projekte',
  },
  skills: {
    label: 'Fähigkeiten & Tech-Stack',
    heading: 'Technologien, Tools & Expertise.',
    subheading: 'Mein Technologie-Ökosystem für Softwareentwicklung, Prozessautomatisierung, E-Commerce und IT-Infrastruktur.',
    categories: {
      all: 'Alle Skills',
      dev: 'Sprachen & Dev',
      web: 'Web, CMS & Shop',
      infra: 'Infrastruktur & Tools',
      biz: 'Management & Business',
    },
    badges: {
      core: 'Hauptstack',
      advanced: 'Erfahren',
      proficient: 'Fundiert',
    },
  },
  cv: {
    label: 'Lebenslauf',
    heading: 'Wer ich bin, wo ich war.',
    subheading: 'Der kurze Weg durch Ausbildung, Beruf und Ehrenamt.',
    work: 'Arbeits- & Ehrenamtliche Erfahrung',
    education: 'Schulbildung',
    volunteer: 'Ehrenamt & Engagement',
    certificates: 'Weiterbildungen',
    skillsInterests: 'Fähigkeiten & Interessen',
    present: 'heute',
    download: 'CV als PDF / Drucken',
    email: 'job@julius-hunold.de',
    website: 'www.julius-hunold.de',
    workItems: [
      {
        role: 'Projektleiter',
        company: 'MD Telekom',
        period: '2022 – heute',
        description: 'Planung, Organisation und Leiten von Glasfaserarbeiten. Entwicklung von branchenspezifischer Software. Delegation und Überwachung von Aufgaben an Mitarbeiter.',
        bullets: [
          'Planung, Organisation und Leiten von Glasfaserarbeiten',
          'Entwicklung von branchenspezifischer Software',
          'Delegation und Überwachung von Aufgaben an Mitarbeiter',
        ],
      },
      {
        role: 'E-Commerce Manager',
        company: 'MD Landmaschinen',
        period: '2022 – 2022',
        description: 'E-Commerce Administration, Amazon, WooCommerce.',
        bullets: [
          'E-Commerce Administration',
          'Amazon, WooCommerce',
        ],
      },
      {
        role: 'Julius Hunold IT-Dienstleistungen',
        company: 'Selbstständig',
        period: '2021 – heute',
        description: 'WaWi-/Onlineshop Einrichtung, Webseiten mit Hosting, IT-Beratung/Schulungen.',
        bullets: [
          'WaWi-/Onlineshop Einrichtung',
          'Webseiten mit Hosting',
          'IT-Beratung/Schulungen',
        ],
      },
      {
        role: 'Aushilfe',
        company: 'Andreas Riesmeier Industrieservice',
        period: '2021',
        description: 'Reinigung und Instandsetzung von Industrieanlagen.',
        bullets: [
          'Reinigung und Instandsetzung von Industrieanlagen',
        ],
      },
      {
        role: 'Gruppenbetreuer / stellv. Öffentlichkeitsarbeitsbeauftragter',
        company: 'Regio Wegweiser',
        period: '2020 – heute',
        description: 'Ausarbeitung und Durchführung von Gruppenstunden mit aktuellen Themen, Repräsentation der Organisation nach außen, Pflegen der Webseite, Audio-/Visuelle Begleitung von Veranstaltungen.',
        bullets: [
          'Ausarbeitung und Durchführung von Gruppenstunden mit aktuellen Themen',
          'Repräsentation der Organisation nach außen, Pflegen der Webseite',
          'Audio-/Visuelle Begleitung von Veranstaltungen',
        ],
      },
    ],
    educationItems: [
      {
        degree: 'Georg-August-Universität Göttingen',
        school: 'Erwarteter Bachelor of Science in Angewandte Informatik 2025',
        period: '2022 – heute',
      },
      {
        degree: 'Werner von Siemens Schule Hildesheim',
        school: 'Allgemeinbildende Hochschulreife 2022',
        period: '2019 – 2022',
      },
      {
        degree: 'Schiller Oberschule Sarstedt',
        school: 'Erweiterter Sekundarabschluss I',
        period: '2018 – 2019',
      },
    ],
    volunteerItems: [
      {
        role: 'Gruppenbetreuer / stellv. Öffentlichkeitsarbeitsbeauftragter',
        org: 'Regio Wegweiser',
        period: '2020 – heute',
      },
      {
        role: 'Gruppenleiter Schulung (JuLeiCa)',
        org: 'Schönstadt Mannesjugend',
        period: '2021',
      },
    ],
    certificateItems: [
      {
        name: 'Gruppenleiter Schulung (JuLeiCa)',
        issuer: 'Schönstadt Mannesjugend | Regio Wegweiser',
        year: '2021',
      },
      {
        name: 'Führerschein Klasse B',
        issuer: 'Fahrschule Kämpfer Hildesheim',
        year: '2019',
      },
      {
        name: 'Besser informiert in die Selbstständigkeit',
        issuer: 'IHK Hannover',
        year: '2019',
      },
    ],
    skills: {
      technologies: 'C, C++, Java, Python, SQL, JSON, JavaScript, JQuery, Linux, MS-Office, Excel',
      youthWork: 'Organisationsfähigkeit, Kommunikativ, Gruppenstunden zu aktuellen Themen',
      interests: 'Betriebswirtschaft, Programmierung, Vertrieb, Projektmanagement',
    },
  },
  contact: {
    label: 'Kontakt',
    heading: 'Lass uns reden.',
    subheading:
      'Hast du ein Projekt im Kopf oder eine Frage? Schreib mir – ich antworte in der Regel innerhalb von 24 Stunden.',
    emailCta: 'E-Mail schreiben',
    phoneCta: 'Telefonat vereinbaren',
    location: 'Sarstedt, Deutschland',
    availability: 'Verfügbar für Freelance-Projekte',
    form: {
      name: 'Dein Name',
      email: 'Deine E-Mail-Adresse',
      message: 'Deine Nachricht',
      submit: 'Nachricht senden',
      sent: 'Danke! Deine Nachricht ist auf dem Weg.',
      error: 'Bitte alle Felder ausfüllen.',
    },
  },
  footer: {
    rights: 'Alle Rechte vorbehalten.',
    builtWith: 'Gebaut mit Astro · GSAP · Tailwind',
    imprint: 'Impressum',
    privacy: 'Datenschutz',
    sites: [
      { label: 'Webdesign', url: 'https://hnld.de' },
      { label: 'Custom Apps', url: 'https://hunold24.de' },
    ],
  },
  common: {
    close: 'Schließen',
    viewDetails: 'View Details',
    readMore: 'Mehr erfahren',
  },
  blog: {
    label: 'Blog',
    heading: 'Notizen & Einblicke.',
    subheading: 'Gedanken zu Web-Entwicklung, Tooling und diesem Projekt.',
    readMore: 'Weiterlesen',
    back: 'Zurück zum Blog',
    published: 'Veröffentlicht am',
  },
  infra: {
    label: 'Verfügbarkeit & Infrastruktur',
    heading: 'Ausfallsicher, schnell & modern gehostet.',
    subheading:
      'Moderne Webanwendungen brauchen ein solides Fundament. Ich sorge dafür, dass Websites und Anwendungen rund um die Uhr schnell, sicher und ohne Ausfallzeiten erreichbar sind.',
    status: {
      online: 'Betriebsbereit',
      degraded: 'Wartung',
    },
    features: [
      {
        title: 'Sicherheit & SSL-Schutz',
        description:
          'Moderne Let’s Encrypt SSL-Verschlüsselung, UFW-Firewalls und isolierte Container schützen Nutzer- und Kundendaten.',
      },
      {
        title: 'Automatische Updates (CI/CD)',
        description:
          'Neuer Code wird bei jedem Push automatisch getestet und ohne Unterbrechungen oder Ausfallzeiten (Zero-Downtime) live eingespielt.',
      },
      {
        title: 'Höchste Performance & Speed',
        description:
          'Durch optimierte Nginx-Webserver und Redis-Zwischenspeicher laden Seiten in Millisekunden und reagieren ohne Verzögerung.',
      },
      {
        title: 'Ausfallsichere Datenbanken',
        description:
          'Strukturierte PostgreSQL-Datenbanken speichern Kundendaten zuverlässig, ausfallsicher und mit Datensicherungen.',
      },
      {
        title: 'Skalierbarer Cloud-Speicher (S3)',
        description:
          'Bilder, Uploads und Dokumente werden in einem hochverfügbaren MinIO S3 Object Storage gespeichert.',
      },
      {
        title: '24/7 Monitoring & Stabilität',
        description:
          'Kontinuierliche Systemprüfungen stellen sicher, dass alle Server stabil laufen und Störungen sofort erkannt werden.',
      },
    ],
  },
  services: {
    label: 'Leistungen & Anfrage',
    heading: 'Entwicklung, Automatisierung & Digitalisierung.',
    subheading:
      'Von der Web-App bis zur vollständigen Prozessautomatisierung – wähle deinen Bereich und starte direkt eine unverbindliche Anfrage.',
    cards: [
      {
        tag: '01 // CUSTOM SOFTWARE',
        title: 'Web-Apps & Prozessautomatisierung',
        description:
          'Maßgeschneiderte Softwarelösungen mit PHP & Python – von komplexer Geschäftslogik und REST-APIs bis zu vollständig automatisierten Workflows und digitalen Prozessen.',
        highlights: ['Laravel & Python Backend', 'Workflow-Automatisierung', 'REST / GraphQL APIs', 'Digitalisierung von Prozessen'],
        badge: 'Automation-First',
      },
      {
        tag: '02 // E-COMMERCE & ERP',
        title: 'Shop-Systeme & digitale Warenwirtschaft',
        description:
          'WooCommerce-Shops mit automatisierten ERP-Anbindungen, digitaler Warenwirtschaft und intelligenten Zahlungsworkflows – weniger Handarbeit, mehr Durchsatz.',
        highlights: ['WooCommerce Customizing', 'ERP & WaWi-Automatisierung', 'Payment-Gateway Integration', 'Conversion-Optimierung'],
        badge: 'Voll automatisiert',
      },
      {
        tag: '03 // DEVOPS & INFRA',
        title: 'IT-Infrastruktur & Cloud-Hosting',
        description:
          'Sichere Linux-Server, Docker-Umgebungen, automatisierte CI/CD-Pipelines und kontinuierliches Monitoring – deine Infrastruktur läuft, ohne dass du daran denken musst.',
        highlights: ['Docker & Linux Server-Setup', 'GitHub Actions CI/CD', 'SSL, Security & Nginx', 'Managed Hosting & IT-Services'],
        badge: 'Self-Hosted',
      },
      {
        tag: '04 // PROJEKT STARTEN',
        title: 'Bereit für dein nächstes Projekt?',
        description:
          'Ob neue Software, automatisierter Workflow oder digitale Infrastruktur – schreib mir direkt und ich melde mich innerhalb von 24 Stunden.',
        cta: 'Anfrage senden →',
        badge: 'Unverbindlich & schnell',
      },
    ],
  },
  cmdPalette: {
    title: 'Schnellnavigation & Befehle',
    placeholder: 'Suchen oder Befehl eingeben (z.B. CV, Kontakt, Projekte)...',
    sections: 'Sektionen',
    actions: 'Schnellaktionen',
    noResults: 'Keine passenden Ergebnisse gefunden.',
    kbdHint: 'Verwende ↑↓ zum Navigieren, Enter zum Auswählen, Esc zum Schließen',
    downloadCv: 'Lebenslauf als PDF herunterladen',
    downloadVcard: 'vCard Kontaktkarte (.vcf) herunterladen',
    copyEmail: 'E-Mail-Adresse kopieren',
    copied: 'In Zwischenablage kopiert! ✓',
    switchLang: 'Sprache auf Englisch wechseln',
    openGithub: 'GitHub Profil öffnen',
    openLinkedin: 'LinkedIn Profil öffnen',
  },
  contactRecruiter: {
    vcardLabel: 'Visitenkarte & Kontaktdaten',
    vcardBtn: 'vCard Visitenkarte (.vcf)',
    copyEmailBtn: 'E-Mail kopieren',
    copied: 'In Zwischenablage kopiert! ✓',
  },
  brands: {
    label: 'Marken & Digital-Studios',
    heading: 'Spezialisierte Expertise für dein digitales Wachstum.',
    hnld: {
      tag: 'hnld.de · Webdesign Studio',
      title: 'High-Performance Webdesign & Markenauftritt',
      subtitle: 'Moderne Websites, die Eindruck hinterlassen – ästhetisch, blitzschnell und für maximale Conversions optimiert.',
      cta: 'hnld.de besuchen',
      highlights: ['Individuelles Webdesign', 'Responsive & Mobile First', 'SEO & Performance'],
    },
    hunold24: {
      tag: 'hunold24.de · Custom Software',
      title: 'Maßgeschneiderte Web-Apps & Cloud-Lösungen',
      subtitle: 'Komplexe Workflows, PHP/Symfony-Backendsysteme, API-Schnittstellen und skalierbare Cloud-Architekturen.',
      cta: 'hunold24.de entdecken',
      highlights: ['Individuelle Software-Apps', 'API-Schnittstellen & Daten', 'Docker & Cloud-Infra'],
    },
    ennery: {
      tag: 'ennery · CleanTech Software',
      title: 'Digitale Energieberatungs­software & Effizienz',
      subtitle: 'Spezialisierte Softwarelösungen für Energieberater – für iSFP-Sanierungsfahrpläne und Gebäude-Audits.',
      cta: 'ennery entdecken',
      highlights: ['iSFP & Sanierungsfahrpläne', 'CO₂- & Effizienz-Rechner', 'Automatisierte Audit-Berichte'],
    },
  },
  socialMarquee: {
    label: 'Profile & Netzwerke im Web',
  },
};

export type Dict = typeof de;
